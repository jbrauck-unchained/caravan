...
| T11 | Package and security | The WebHID dependency's `@sentry/minimal` capture calls transmit unexpected data. | Do not initialize Sentry; inspect exact packed behavior and data paths; require privacy and agreement approval; record decision in authorization evidence. | Dependency source/bundle/network-destination audit before live testing or publication. | Behavior can change with dependency updates and requires re-review. |
